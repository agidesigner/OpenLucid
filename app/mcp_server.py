...
@mcp.tool()
async def refine_creation_sections(
    creation_id: str,
    ...
    """LLM-driven refinement of selected sections of a creation, preserving
    the rest. Use this when the user is iterating ("rewrite the hook with
    a data point" / "make the body punchier" / "soften the CTA") instead
    of regenerating the entire piece — keeps the parts they liked.

    The creation MUST have ``structured_content`` with sections (i.e. it
    came from script_writer / content_studio composer mode, not a manual
    plain-text creation). Discover valid section ids via get_creation,
    then look at structured_content.section_ids.

    Sections you do not list in ``section_ids`` are guaranteed to stay
    byte-identical. ``constraint`` is the natural-language instruction
    sent to the LLM (e.g. "更口语化", "add a stat in the first sentence").

    Optional config_id / model_override let you pick a specific LLM
    config; otherwise the script_writer scene default is used. ``language``
    forces output language; otherwise auto-detected from existing content.

    Returns the updated CreationResponse with the refined sections plus
    a regenerated plain-text ``content`` mirror.
    """
    from app.application.creation_service import CreationService
    from app.schemas.creation import CreationResponse
...
@mcp.tool()
async def update_creation_section(
    creation_id: str,
    section_id: str,
    new_text: str,
) -> str:
    """Manual (no-LLM) edit of a single section's text. Use for typo
    fixes, swapping a number, removing a sentence — anything where you
    know the exact replacement text and don't need the LLM. Faster and
    cheaper than refine_creation_sections.

    The plain-text ``content`` mirror is regenerated from the updated
    structured_content in the same transaction; both stay in sync.

    Returns the updated CreationResponse.
    """
    from app.application.creation_service import CreationService
    from app.schemas.creation import CreationResponse
    ...
    async with _session_factory() as session:
        ...
        creation = await svc.update_section_text(
            uuid.UUID(creation_id), section_id, new_text,
        )
        await session.commit()
        await session.refresh(creation)
        return _serialize(creation, CreationResponse)
...
async def match_broll_assets(
    ...
    """For each B-roll shot in a video script creation, find the best
    existing asset in the user's library that matches — or null if no
    asset clears the strict gates (≥720p short edge, duration 2-15s
    for video, aspect must exactly match ``aspect``, semantic match
    confirmed by LLM rerank).
    ...
    Args:
      creation_id: UUID of the creation. Must have structured_content
        with a broll_plan and an offer_id (asset library is offer-scoped).
      aspect: Target video aspect — "portrait" / "landscape" / "square"
        or canonical "9:16" / "16:9" / "1:1".

    Returns JSON: {"matches": [{"shot_index": int, "matched_asset":
    {...} | null}, ...]}. The matched_asset shape includes id, file_name,
    preview_uri, asset_type, duration_seconds, width, height, aspect,
    resolution_label."""
    import json as _json
    from app.application.broll_matching_service import match_assets_for_broll

    async with _session_factory() as session:
        matches = await match_assets_for_broll(
            session, uuid.UUID(creation_id), aspect,
        )
        return _json.dumps({"matches": matches}, ensure_ascii=False)
