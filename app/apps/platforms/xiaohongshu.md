...
**禁用词**（会被算法降权）：立即购买、优惠券、下单领取、点击链接、扫码咨询、X折、秒杀、限时抢购、添加客服、私信购买。任何直接销售话术都要删除 —— 小红书的生态只允许**"暗示购买路径"**，不允许明说。

---

**标题公式（决定 80% 点击率）—— 选一个执行，不要混**：

- **身份 + 数字 + 情绪动词**：`女大学生自费1k怒买15件pphome`、`宝妈熬夜3天做出的万圣节套装`
  （身份前置占 3-5 字；"怒买/疯抢/偷吃/连吃3碗"这类强情绪动词胜过中性词）

- **"不+负面"表达正向**：`五一拍照不挨骂`、`送礼不踩雷的10个选择`、`做饭不翻车的懒人清单`
  （用户对"不要什么"比"要什么"更敏感）

- **反问 / 调侃式**：`凭咱两的关系 端午请我吃个粽子 很过分吗`、`这个面膜谁用谁长公主`
  （反问在标题里天然邀请评论，互动率翻倍）

- **多品类堆叠**：`深夜偷吃｜螺蛳粉奶茶火锅一次全`、`又鲜又营养‼️巨好喝✅`
  （3 个以上并列制造冲击，用 ｜ / ‼️ / ✅ 当断句）

- **连载系列**：`挑战不开店做100个蛋糕-连载vol.11`、`装修记录｜Day 23 终于贴砖了`
  （追更机制让老粉 loop 回来）

- **超短无承诺**：`女友和我的日常`、`秋天第一杯奶茶`（关系内容/仪式感场景只要 6-10 字就够）

- **传统老公式仍适用**（优先级低）：数字+结果（`用了3个月皮肤变成这样`）、反差好奇（`月薪8000住理想的家`）、避雷救命（`别再踩坑！这3个误区我全中`）

---

**标点与 emoji 用法（和其他平台最大的差异）**：

- Emoji 是**情感坐标**，不是装饰 —— 每个有固定角色：
  - ‼️ = 强调、兴奋，常替代感叹号
  - ✅ = 确认推荐
  - ⚠️ = 避雷、警告
  - 💡 = 灵感、tips
  ...
  - 🌱 = 种草
  - 🧡 / 🩷 = 女性向品类
- Emoji **可以当断句**代替逗号：`巨鲜美 | 超好喝✅` 比 `巨鲜美，超好喝` 更小红书
- 每段 1-2 个 emoji 就够，**堆砌反而掉价**（连续三个 emoji 会被判定为营销号）
- `｜` 竖线用来并列短语，`/` 用来轻快转折
...
- **首屏 = 场景化首句**（不是背景铺垫）：具体时间 + 地点 + 动作 > 抽象感叹
  - ✅ `昨晚熬到2点剪视频，头发都乱了`
  - ❌ `视频创作真的很辛苦`
- **中段 = 列表格式**占绝对主力（3/5/7/10 条）。整段文字墙在小红书几乎必跳出。
- 列表小标题建议 emoji 前缀（🌱 种草项、⚠️ 踩坑项、💡 技巧项）
- 对比段用双栏或双项："✨优点：xxx  / ⚠️缺点：xxx"
- **结尾 = 轻互动问句**："姐妹们试过吗？" / "还有什么好物评论区告诉我" —— 引发评论而非 CTA。直接号召行动容易被限流。

---

**按内容类型差异化（辅助选择）**：

- **种草帖 / 好物分享**：身份开场 → 入坑契机 → 3-5 条具体好用点（带细节）→ 真实使用感受 → 邀请评论
- **避雷帖 / 踩坑复盘**：痛点切入 → "我踩过的N个坑" 列表 → 正确做法对比 → "希望看到的姐妹少交智商税"
- **教程 / 保姆级**：谁适合看（人群定位）→ 所需清单 → 步骤 1-5 带实拍图位 → 常见翻车提醒 → 成品效果
- **日常 vlog / 情感**：时间戳开场 → 2-3 段短场景 → 自我感悟一句 → 留白式结尾（不问问题，像日记自然收尾）
- **对比测评**：3-5 个产品的横向表格或分段评测 → 最终"最推"和"最劝退"判决

---

**Hashtag 规范**：3-6 个。**越精准越好**（精准长尾词 > 大词）。格式：`#品类词 #效果词 #场景词 #身份词`。
  - ✅ `#干皮面霜 #油皮友好 #秋冬必备 #打工人护肤`
  - ❌ `#美妆 #护肤 #好物` （大词已经泛滥，几乎不带量）

**图片提示（image_hint）**：每个 section 描述理想配图。要具体：
  - ✅ "产品与晨光中的咖啡平铺，木质桌面，暖黄色调"
  - ❌ "产品图"
封面图要**有生活感**（手持 / 使用中 / 与食物一起），纯产品摄影封面在小红书点击率低 50%+。
