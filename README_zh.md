...
**Q：忘记管理员密码 / 失去原邮箱访问权限 / 想用脚本一键创建首位管理员？**
看 [自部署运维指南](SELF_HOSTING_zh.md#账号恢复) —— 有一条直连 DB 的 CLI（`docker compose exec app python -m app.cli ...`），绕开邮件和 HTTP 鉴权，专门处理这类恢复场景。

**Q：能不能自定义各平台（公众号 / 博客 / 小红书 / ...）的写作规范、人设语气、叙事结构？**
能 —— 每个平台 / 人设 / 叙事结构 / 内容形态 / 营销活动类型的提示词都是一个 markdown 文件，你可以直接改。shipped 默认在 git 里，你的 customize 在 docker volume 里 —— `git pull` 不会冲突。详见 [SELF_HOSTING_zh.md#自定义内容生成提示词](SELF_HOSTING_zh.md#自定义内容生成提示词)。
